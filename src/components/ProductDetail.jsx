...
import { Container, Row, Col, Card, Button, Carousel, Navbar, Modal } from 'react-bootstrap';
  ...
  return (
    <div>
      ...
      <Container className="my-5">
        <Row>
          <Col md={6}>
            <Card>
                ...
                <Carousel>
                  {product.images.map((image, index) => (
                    <Carousel.Item key={index}>
                      <Card.Img variant="top" src={image} alt={product.name} className="img-fluid" />
                    </Carousel.Item>
                  ))}
                </Carousel>
              ) : (
              ...
              <Card.Body>
                ...
                <Button variant="success" className="w-100" onClick={handleShowModal}>Buy Now</Button>
          ...
          <Col md={6}>
            <Card className="p-3">
              <Card.Body>
                <Card.Title className="bg-dark text-white p-2 rounded" style={{ marginBottom: '2rem' }}>{product.name}</Card.Title>
                <Card.Text style={{ textAlign: 'left' }}><strong>Brand:</strong> {product.brand}</Card.Text>
                <Card.Text style={{ textAlign: 'left' }}><strong>Price:</strong> <span className="bg-warning text-dark p-1 rounded">{product.currentPrice}</span></Card.Text>
                <Card.Text style={{ textAlign: 'left' }}><strong>Replacement:</strong> {product.returnPolicy}</Card.Text>
                <Card.Text style={{ textAlign: 'left' }}><strong>Warranty:</strong> {product.warranty}</Card.Text>
                ...
                <ul>
                  {product.problemItSolves.map((problem, index) => (
                    <li key={index} style={{ textAlign: 'left' }}>{problem}</li>
                  ))}
                </ul>
                <hr />
                ...
                <ul>
                  {product.advantages.map((adv, index) => (
                    <li key={index} style={{ textAlign: 'left' }}>{adv}</li>
                  ))}
                </ul>
                <hr />
                ...
                <ul>
                  {product.disadvantages.map((disadv, index) => (
                    <li key={index} style={{ textAlign: 'left' }}>{disadv}</li>
                  ))}
                </ul>
                <hr />
                <h4 className="text-primary">Special Features</h4>
                <ul>
                ...
                <ul>
                  {product.includedComponents.map((component, index) => (
                    <li key={index} style={{ textAlign: 'left' }}>{component}</li>
                  ))}
                </ul>
                <hr />
                ...
                <ul>
                  {product.recommendedUses.map((use, index) => (
                    <li key={index} style={{ textAlign: 'left' }}>{use}</li>
                  ))}
                </ul>
                <Button variant="success" className="mt-3" onClick={handleShowModal}>Buy Now</Button>
              </Card.Body>
            </Card>
          </Col>
        </Row>
      ...
      <Modal show={showModal} onHide={handleCloseModal} centered>
        ...
        <Modal.Body>
          <Button variant="primary" className="w-100 mb-3" href={product.amazonUrl} target="_blank">Buy on Amazon</Button>
          <Button variant="primary" className="w-100" href={product.flipkartUrl} target="_blank">Buy on Flipkart</Button>
        </Modal.Body>
      </Modal>
